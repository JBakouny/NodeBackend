...
dishRouter.route('/:dishId/comments/:commentId')
    ...
    .put(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
        Dishes.findById(req.params.dishId)
            // .populate('comments.author') not necessary since we only need the id here
            .then((dish) => {
                if (dish !== null && dish.comments.id(req.params.commentId) !== null){
                    // comparing 2 MongoDB ids should be done using equals:
                    // https://medium.com/@mzndako/comparing-mongoose-object-id-often-fail-a7374a779f6d
                    // You can also compare by converting to strings:
                    // https://whitehorsesblogarchive.wordpress.com/2017/10/15/how-to-compare-mongo-_ids-in-javascript/
                    let author_id = dish.comments.id(req.params.commentId).author
